...
export function ModalImpresion({ pedido, productos, personalizados = [], filamentos, insumos = [], onClose, onDone }) {
  ...
  // El mismo hook que usa el paso de tomar el pedido, división entre owners
  // incluida. Dos diferencias: acá se evalúa contra el stock FÍSICO y no el
  // neto de reservas —se está imprimiendo, y restar la reserva de este mismo
  // pedido sería contarla dos veces— y arranca precargado con lo que se
  // reservó, reparto y todo, editable como si se armara de cero.
  const o = usarOrigen({ plan, filamentos, desperdicios, inicial: pedido?.origen });
  const {
    planResuelto, totalPorLinea, candidatos, origen, asignaciones,
      ...
      >
        ...
        {plan.length === 0 ? (
          ...
          <>
            ...
            <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: 1.2, textTransform: "uppercase", color: "var(--muted)", marginBottom: 12 }}>
              Material a descontar
            </div>
            <LineasDeMaterial o={o} modo="imprimir"
              desperdicios={desperdicios} setDesperdicios={setDesperdicios}/>
          </>
        )}
